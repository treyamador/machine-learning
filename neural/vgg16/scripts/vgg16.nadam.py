...
def train_VGG16(model, epochs):
    ...
    for i in range(0, len(train_paths), 1000):
        print(train_paths[i], train_ages[i])
        cv2.imshow('', cv2.imread(train_paths[i]))
        cv2.waitKey(0)

    for i in range(0, len(val_paths), 1000):
        print(val_paths[i], val_ages[i])
        cv2.imshow('', cv2.imread(val_paths[i]))
        cv2.waitKey(0)
