...
def get_steps(x_path):
    ...
    return math.ceil((sum(slcs)/2) / BATCH_SIZE)
...
def gen_samples(rng):
    gauss = signal.windows.gaussian(rng, rng/3)
    gauss /= sum(gauss)
    ln = rng//2 + BATCH_SIZE-((rng//2) % BATCH_SIZE)
    samples = np.random.choice(range(rng), ln, p=gauss, replace=False)
    random.shuffle(samples)
    return np.ndarray.tolist(samples)
...
def generate_data(x_path, y_path, transf=None):
    ...
    while True:
        ...
        for smpl in samples:
            ...
            sections = gen_samples(flair.shape[2])
            sections = [sections[batch*i: batch*i+batch] for i in range(flair.shape[2]//batch)]
            ...
            for scts in sections:
                ...
                for i, s in enumerate(scts):
                    flair_sect = normalize(flair[:, :, s])
                    t1_sect = normalize(t1[:, :, s])
